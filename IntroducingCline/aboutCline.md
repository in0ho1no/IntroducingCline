...
ClineはVSCodeで動作するAI開発アシスタント拡張機能です。複数のAIプロバイダーに対応しており、自然言語による指示からコードの読み取り、編集、ファイル作成、コマンド実行などを自動化できます。
...
ローカル実行とは、LLM（大規模言語モデル）をインターネット経由ではなく、自分のPC上で直接動作させる方法です。機密性の高いコードでも外部に送信せずに安全に利用できます。
...
1. **ファイル名変更時**: マッピング情報を更新  
1. **月一回**: 設定ファイルの見直し

## 制限事項

1. **生成されたコードは必ずレビューが必要**: Clineは提案ベースの支援を行うため
